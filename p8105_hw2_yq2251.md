...
\#Question 2 \#use separate function to separate “mon” and create
variable “president”
...
\#problem 3: read “Names” dataset and rename categorical names
...
```
    ...
    ## Warning: Values in `Rank` are not uniquely identified; output will contain list-cols.
    ## * Use `values_fn = list(Rank = list)` to suppress this warning.
    ## * Use `values_fn = list(Rank = length)` to identify where the duplicates arise
    ## * Use `values_fn = list(Rank = summary_fun)` to summarise duplicates
...
WMNH_PLOT + geom_point() + geom_text(data=subset(WMNH2016N, Rank<2))+ggtitle("Male Baby name ranking in White,non-Hispanic 2016")
```

![](p8105_hw2_yq2251_files/figure-gfm/unnamed-chunk-20-1.png)<!-- -->
